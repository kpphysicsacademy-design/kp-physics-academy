...
(function(){
 ...
 function route(name){
  ...
  if(subject && !/[?&]subject=/.test(target) && ["home","practice","compete"].includes(name)){
    target+=(target.includes("?")?"&":"?")+"subject="+encodeURIComponent(subject);
  }
  return target;
 }
 function detectActive(){const p=(location.pathname.split("/").pop()||"index.html").toLowerCase();if(p==="index.html"||p==="skillup-home.html")return"home";if(p==="practice.html")return"practice";if(["compete.html","league.html","challenge.html","challenge-concepts.html"].includes(p))return"compete";if(p==="profile.html")return"profile";return"";}
 window.renderSkillUpNavigation=function(active){document.querySelectorAll(".skillup-bottom-nav").forEach(n=>n.remove());const nav=document.createElement("nav");nav.className="skillup-bottom-nav";nav.setAttribute("aria-label","SkillUp main navigation");const items=[["home","Home"],["practice","Practice"],["compete","League"],["profile","Profile"]];nav.innerHTML=items.map(([k,label])=>'<a href="'+route(k)+'" class="'+(active===k?"active":"")+'"><span class="skillup-nav-icon" aria-hidden="true">'+icons[k]+'</span><span>'+label+'</span></a>').join("");document.body.appendChild(nav);};
 function install(){
  if(!document.getElementById("skillup-nav-style")){
   ...
   .skillup-bottom-nav a{min-width:0!important;flex:1 1 25%!important;max-width:25%!important;text-decoration:none!important;color:#6b7280!important;font-size:10px!important;font-weight:800!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;gap:2px!important;line-height:1!important;padding:0!important;margin:0!important}
