...
(function(){
 ...
 window.renderSkillUpNavigation=function(active){
  ...
  nav.innerHTML=items.map(([k,label])=>'<a href="'+route(k)+'" class="'+(active===k?"active":"")+'">'+((k==="learn"||k==="practice"||k==="compete")?'':'<span class="skillup-nav-icon" aria-hidden="true">'+icons[k]+'</span>')+'<span>'+label+'</span></a>').join("");
